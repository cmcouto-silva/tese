...
# VCF File
vcf_file <- "../data/phased_adj_alleles/dataset_S1.vcf.gz"
...
pop_subset <- populations[SelGroup!="", paste0(FID, ":", IID)]
# Get list of target-populations
target_pops_names <- populations[SelGroup!="", unique(SelGroup)]
target_pops <- sapply(target_pops_names, function(pop) {
  populations[SelGroup==pop, paste0(FID, ":", IID)]
...
dir.create("data", F)
subset_vcf(vcf_file, populations[SelGroup!="", paste0(FID, ":", IID)], out = vcf_subfile)
...
scanhh_list <- sapply(target_pops_names, function(pop){
  ...
  # scanhh_chrlist <- lapply(haplohh_list[[pop]], function(haplohh){
  #   scan_hh(haplohh)
  #   # scan_hh(haplohh, threads = threads) # parallelization
  # })
  return(scanhh)
}, simplify = F)
...
# Read scanhh file
if(!"scanhh_list" %in% ls()) scanhh_list <- readRDS("bin/scanhh_list.RDS")
