...
os.chdir('dataset_S2/gtex_annot/')
...
ds2_persnp = {method: pd.read_feather(f'../results_bin/persnp/{method}.feather') for method in methods}
ds2_persnp_top = {method: get_top(ds2_persnp[method], .999) for method in methods}

# Load Phased Data
vcf = h5py.File('../data/phased_adj_alleles/dataset_S2.h5', 'r')
...
pop_selection = pop.sel_group.isin(['EastAsia', 'Mesoamerica', 'Amazonia'])
...
groups = {
    ...
    'amz': pop[pop.sel_group == 'Amazonia'].index.tolist(),
    'mes': pop[pop.sel_group == 'Mesoamerica'].index.tolist(),
    'eas': pop[pop.sel_group == 'EastAsia'].index.tolist()
}
...
ds2_persnp_eqtl_df = dict()
ds2_persnp_eqtl_dict = dict()
...
for method in methods:
    ...
    eqtl_input_df = df_freq[df_freq.SNP.isin(ds2_persnp_top[method].SNP)].copy()
    ...
    eqtl_input_df = eqtl_input_df.merge(ds2_persnp_top[method][['SNP','GENE']])
    ...
    ds2_persnp_eqtl_df[method] = eqtl_df
    ds2_persnp_eqtl_dict[method] = eqtl_dict
...
with open('eqtl_persnp_df.pickle', 'wb') as file:
    pickle.dump(ds2_persnp_eqtl_df, file)

with open('eqtl_persnp_dict.pickle', 'wb') as file:
    pickle.dump(ds2_persnp_eqtl_dict, file)
